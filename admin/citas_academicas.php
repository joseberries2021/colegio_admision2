...
if (isset($_GET['aprobar']) && isset($_GET['id'])) {
    ...
    if ($cita && $cita['tipo'] == 'academica') {
        ...
        registrarAuditoria('citas_academicas', 'aprobar', 'cita', $id, 'Cita académica aprobada para postulante ID: ' . $cita['id_postulante']);
        $mensaje = "✅ Cita académica aprobada correctamente";
    }
    
    header('Location: citas_academicas.php');
    exit;
}

// ============================================
// RECHAZAR CITA ACADÉMICA
// ============================================
if (isset($_GET['rechazar']) && isset($_GET['id'])) {
    ...
    if ($cita && $cita['tipo'] == 'academica') {
        ...
        query("UPDATE postulantes SET estado_proceso = 'cita_aprobada' WHERE id = ?", [$cita['id_postulante']]);
        registrarAuditoria('citas_academicas', 'rechazar', 'cita', $id, 'Cita académica rechazada para postulante ID: ' . $cita['id_postulante']);
        $mensaje = "❌ Cita académica rechazada";
    }
...
// ============================================
// MARCAR CITA ACADÉMICA COMO REALIZADA
// ============================================
if (isset($_GET['realizada']) && isset($_GET['id'])) {
    ...
    if ($cita && $cita['tipo'] == 'academica') {
        ...
        registrarAuditoria('citas_academicas', 'realizada', 'cita', $id, 'Cita académica marcada como realizada para postulante ID: ' . $cita['id_postulante']);
        $mensaje = "✅ Cita académica marcada como realizada";
    }
...
if (isset($_GET['eliminar']) && isset($_GET['id'])) {
    ...
    if ($cita && $cita['tipo'] == 'academica') {
        ...
        $citas_restantes = fetchOne("SELECT COUNT(*) as total FROM citas WHERE id_postulante = ? AND estado != 'cancelada' AND tipo = 'academica'", [$cita['id_postulante']]);
        if ($citas_restantes['total'] == 0) {
            query("UPDATE postulantes SET estado_proceso = 'cita_aprobada' WHERE id = ?", [$cita['id_postulante']]);
        }
        registrarAuditoria('citas_academicas', 'eliminar', 'cita', $id, 'Cita académica eliminada para postulante ID: ' . $cita['id_postulante']);
        $mensaje = "🗑️ Cita académica eliminada correctamente";
    }
    header('Location: citas_academicas.php');
    exit;
}
...
$where = "1=1 AND c.tipo = 'academica'";
$params = [];
...
// Obtener citas pendientes (académicas)
$citas_pendientes = fetchAll("
    ...
    WHERE c.estado = 'pendiente' AND c.tipo = 'academica'
    ORDER BY c.fecha ASC, c.hora ASC
");

// Obtener todas las citas académicas (con filtros)
...
$stats = fetchOne("
    SELECT 
        (SELECT COUNT(*) FROM citas WHERE estado = 'pendiente' AND tipo = 'academica') as pendientes,
        (SELECT COUNT(*) FROM citas WHERE estado = 'confirmada' AND tipo = 'academica') as confirmadas,
        (SELECT COUNT(*) FROM citas WHERE estado = 'realizada' AND tipo = 'academica') as realizadas,
        (SELECT COUNT(*) FROM citas WHERE estado = 'cancelada' AND tipo = 'academica') as canceladas,
        (SELECT COUNT(*) FROM citas WHERE tipo = 'academica') as total
");
...
<div class="row">
    <div class="col-md-12">
        <h4><i class="bi bi-pencil-square"></i> Citas Académicas</h4>
        <p class="text-muted">Administra las citas de evaluación académica de los postulantes</p>
        ...
        <div class="card-dashboard mb-4">
            ...
            <?php if (empty($citas_pendientes)): ?>
                <div class="text-center py-4 text-muted">
                    ...
                    <p>No hay citas académicas pendientes de confirmación</p>
                </div>
            <?php else: ?>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div class="btn-group btn-group-sm">
                                            <a href="citas_academicas.php?aprobar=1&id=<?php echo $c['id']; ?>" 
                                               class="btn btn-success"
                                               onclick="return confirm('¿Aprobar esta cita académica?')">
                                                <i class="bi bi-check"></i> Aprobar
                                            </a>
                                            <a href="citas_academicas.php?rechazar=1&id=<?php echo $c['id']; ?>" 
                                               class="btn btn-danger"
                                               onclick="return confirm('¿Rechazar esta cita académica?')">
        ...
        <!-- ========================================== -->
        <!-- HISTORIAL DE CITAS ACADÉMICAS -->
        <!-- ========================================== -->
        <div class="card-dashboard">
            <div class="d-flex justify-content-between align-items-center">
                <h6 class="text-primary-dark"><i class="bi bi-list"></i> Historial de Citas Académicas</h6>
            ...
            <form method="GET" class="row g-2 mb-3">
                ...
                <div class="col-md-2">
                    <a href="citas_academicas.php" class="btn btn-outline-secondary btn-sm w-100">
            ...
            <?php if (empty($citas_todas)): ?>
                <div class="text-center py-4 text-muted">
                    ...
                    <p>No hay citas académicas con los filtros seleccionados</p>
                </div>
            <?php else: ?>
                <div class="table-responsive">
                    <table class="table table-hover">
                                    ...
                                    <td>
                                        <div class="btn-group btn-group-sm">
                                            <?php if ($c['estado'] == 'confirmada'): ?>
                                                <a href="citas_academicas.php?realizada=1&id=<?php echo $c['id']; ?>" 
                                                   class="btn btn-info"
                                                   onclick="return confirm('¿Marcar esta cita como realizada?')">
                                                    <i class="bi bi-check2-circle"></i> Realizada
                                                </a>
                                            <?php endif; ?>
                                            <a href="citas_academicas.php?eliminar=1&id=<?php echo $c['id']; ?>" 
