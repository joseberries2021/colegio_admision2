...
// ============================================
// VERIFICAR QUE LA CITA PSICO ESTÉ COMPLETADA PARA ACCEDER A ACADÉMICA
// ============================================
...
// Si la cita psicopedagógica NO está completada, redirigir
if (!$psico_completada) {
    header('Location: cita.php?id=' . $postulante_id . '&error=psico_pendiente');
    exit;
}

// Si no requiere evaluación, redirigir a matrícula
...
// ============================================
// PROCESAR SOLICITUD DE CITA ACADÉMICA
// ============================================
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['agendar_academica'])) {
    $fecha = $_POST['fecha_academica'] ?? '';
    $hora = $_POST['hora_academica'] ?? '';
    
    if (empty($fecha) || empty($hora)) {
        $error = "❌ Por favor selecciona una fecha y hora para la evaluación académica.";
    } else {
        // Validar que la hora esté disponible
        $existe = fetchOne("SELECT id FROM citas WHERE fecha = ? AND hora = ? AND tipo = 'academica' AND estado != 'cancelada'", 
                           [$fecha, $hora]);
        ...
        if ($existe) {
            ...
            $insertado = insert(
                "INSERT INTO citas (id_postulante, tipo, fecha, hora, estado) VALUES (?, 'academica', ?, ?, 'pendiente')",
                [$postulante_id, $fecha, $hora]
            );
            
            if ($insertado) {
                query("UPDATE postulantes SET estado_proceso = 'evaluacion_pendiente' WHERE id = ?", [$postulante_id]);
                $mensaje = "✅ Cita académica reservada exitosamente. Espera la confirmación del administrador.";
                $cita_academica = fetchOne("SELECT * FROM citas WHERE id_postulante = ? AND tipo = 'academica' ORDER BY id DESC LIMIT 1", [$postulante_id]);
            } else {
                $error = "❌ Error al agendar la cita académica. Intenta nuevamente.";
            }
        }
    }
}
...
// ============================================
// PROCESAR CANCELACIÓN DE CITA ACADÉMICA
// ============================================
if (isset($_GET['cancelar']) && isset($_GET['id'])) {
    ...
    if ($cita && $cita['tipo'] == 'academica') {
        ...
        $cita_academica = null;
        $mensaje = "✅ Cita académica cancelada. Puedes agendar una nueva.";
    }
}
...
if (isset($_GET['action']) && $_GET['action'] == 'horas') {
    ...
    if (empty($fecha) || empty($tipo) || $tipo != 'academica') {
        echo json_encode([]);
        exit;
    }
    
    $citas = fetchAll("SELECT hora FROM citas WHERE fecha = ? AND tipo = 'academica' AND estado != 'cancelada'", [$fecha]);
...
<head>
    ...
    <title>Evaluación Académica - Admisión 2027</title>
    ...
    <style>
        ...
        .horas-grid { display: grid; grid-template-columns: repeat(4, 1fr); gap: 10px; margin-top: 15px; }
    ...
    <div class="card" style="max-width: 850px; margin: 0 auto;">
        <div class="card-body p-4">
            ...
            <div class="estado-pasos">
                ...
                <div class="estado-paso actual"><i class="bi bi-arrow-right-circle"></i><br>EVALUACIÓN ACADÉMICA</div>
            ...
            <div class="card-cita">
                <div class="d-flex justify-content-between align-items-center">
                    ...
                    <div>
                        <span class="badge bg-<?php echo $postulante['estado_proceso'] == 'evaluacion_pendiente' ? 'warning' : 'success'; ?>">
            ...
            <!-- ========================================== -->
            <div class="card-cita">
                <h5 class="text-primary-dark"><i class="bi bi-pencil-square"></i> EVALUACIÓN ACADÉMICA</h5>
                <p class="text-muted">Evaluación de conocimientos. Se realiza los <strong>Sábados</strong> de <strong>8:30 a 12:00</strong>.</p>
                <p class="text-muted"><strong>Duración:</strong> 45 minutos.</p>
                <hr>

                <?php if ($cita_academica && $cita_academica['estado'] != 'cancelada'): ?>
                    <!-- CITA ACADÉMICA YA RESERVADA -->
                    <div class="confirmacion-cita">
                        <h5><i class="bi bi-check-circle-fill text-success"></i> CITA ACADÉMICA RESERVADA</h5>
                        <p class="detalle"><strong><?php echo date('l, d \d\e F', strtotime($cita_academica['fecha'])); ?></strong></p>
                        <p class="detalle"><strong>Horario reservado:</strong> <?php echo date('h:i A', strtotime($cita_academica['hora'])); ?></p>
                        <p class="detalle"><strong>Duración:</strong> 45 minutos</p>
                        <p class="detalle"><strong>Modalidad:</strong> Evaluación Presencial</p>
                        <div class="mt-3">
                            <span class="badge bg-<?php echo $cita_academica['estado'] == 'confirmada' ? 'success' : 'warning'; ?>">
                                <?php echo $cita_academica['estado'] == 'confirmada' ? '✅ Confirmada' : '⏳ En revisión'; ?>
                            </span>
                        </div>
                    </div>
                    <div class="mt-3 d-flex gap-2">
                        <?php if ($cita_academica['estado'] == 'pendiente'): ?>
                            <a href="academica.php?id=<?php echo $postulante_id; ?>&cancelar=<?php echo $cita_academica['id']; ?>" 
                               class="btn btn-outline-danger" onclick="return confirm('¿Cancelar esta cita académica?')">
                                <i class="bi bi-x-circle"></i> Cancelar Cita
                            </a>
                            <a href="academica.php?id=<?php echo $postulante_id; ?>" class="btn btn-outline-secondary">
                                <i class="bi bi-arrow-clockwise"></i> Reagendar Cita
                            </a>
                        <?php endif; ?>
                        <?php if ($cita_academica['estado'] == 'confirmada'): ?>
                            <a href="matricula.php?id=<?php echo $postulante_id; ?>" class="btn btn-success">
                                <i class="bi bi-arrow-right"></i> Continuar a Matrícula
                            </a>
                        <?php endif; ?>
                    </div>
                <?php else: ?>
                    <!-- AGENDAR NUEVA CITA ACADÉMICA -->
                    <form method="POST">
                        <h6 class="text-primary-dark">1. SELECCIONE UNA FECHA (SÁBADOS)</h6>
                        <div class="calendario-container">
                            <input type="text" id="calendarioAcademico" class="form-control" placeholder="Selecciona una fecha..." readonly>
                            <small class="text-muted">Horario: 8:30 - 12:00 (Duración: 45 min)</small>
                        </div>

                        <div class="mt-3" id="horasAcademicoContainer" style="display: none;">
                            <h6 class="text-primary-dark">2. SELECCIONE UN HORARIO DISPONIBLE</h6>
                            <div class="horas-grid" id="horasAcademicoGrid">
                                <!-- Se llena con JavaScript -->
                            </div>
                            <input type="hidden" name="fecha_academica" id="fechaAcademicaSeleccionada" value="">
                            <input type="hidden" name="hora_academica" id="horaAcademicaSeleccionada" value="">
                            <button type="submit" name="agendar_academica" class="btn btn-success mt-3 w-100" id="btnAgendarAcademico" disabled>
                                <i class="bi bi-calendar-check"></i> Reservar Cita Académica
                            </button>
                        </div>
                    </form>
                <?php endif; ?>
...
document.addEventListener('DOMContentLoaded', function() {
    // ============================================
    // CONFIGURACIÓN DE FLATPICKR PARA ACADÉMICA (SÁBADOS)
    // ============================================
    <?php if (!$cita_academica || $cita_academica['estado'] == 'cancelada'): ?>
    flatpickr("#calendarioAcademico", {
        locale: "es",
        dateFormat: "Y-m-d",
        minDate: "today",
        ...
        onChange: function(selectedDates, dateStr, instance) {
            if (dateStr) {
                document.getElementById('fechaAcademicaSeleccionada').value = dateStr;
                cargarHorasAcademico(dateStr);
            }
        }
    });

    function cargarHorasAcademico(fecha) {
        const container = document.getElementById('horasAcademicoContainer');
        const grid = document.getElementById('horasAcademicoGrid');
        const btn = document.getElementById('btnAgendarAcademico');
        ...
        // Horarios: 8:30-12:00 (duración 45 min, intervalos de 45 min)
        const horas = ['08:30:00', '09:15:00', '10:00:00', '10:45:00', '11:30:00'];
        
        fetch('academica.php?id=<?php echo $postulante_id; ?>&action=horas&fecha=' + fecha + '&tipo=academica')
                ...
                horas.forEach(hora => {
                    ...
                    if (!ocupada) {
                        ...
                        div.onclick = function() {
                            ...
                            document.getElementById('horaAcademicaSeleccionada').value = hora;
                            btn.disabled = false;
                        };
                    }
